...
def courbe_stat(capteur, variable, start_date, end_date):
    ...
    date_stop = datetime.datetime.strptime(start_date, '%Y-%m-%d') + datetime.timedelta(days = 1)   #mise en place d'une variable qui permet de ne prendre qu'un jour de mesure à la fois pour le calcul des différentes moyennes, et qui est incrémenté jusqu'à la end_date
    k = 0
...
def superposition_courbes(capteur_1, capteur_2, variable, start_date, end_date):
    ...
    plt.title('blue : capteur_1, red : capteur_2')
    plt.show()
...
def similarites(capteur_1, capteur_2, variable, start_date, end_date):
    ...
    for k in range(3, n-3):
        if variables_triees[k-3] <= variables_triees[k] <= variables_triees[k+3] or variables_triees[k-3] >= variables_triees[k] >= variables_triees[k+3]:
            plt.scatter(dates_triees[k], variables_triees[k], color = 'yellow')
        elif indicateur_trie[k] == 1:
            plt.scatter(dates_triees[k], variables_triees[k], color = 'blue', marker = '+')
        else:
            plt.scatter(dates_triees[k], variables_triees[k], color = 'red', marker = '+')
    plt.title('blue : capteur_1, red : capteur_2, yellow : common points')
    plt.show()
